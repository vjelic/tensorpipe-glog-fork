...
struct CudaStreamDeleter {
  void operator()(hipStream_t stream) {
    TP_CUDA_CHECK(hipStreamDestroy(stream));
  }
};

using CudaTensor = std::unique_ptr<uint8_t[], CudaMemoryDeleter>;
using CudaStream =
    std::unique_ptr<std::remove_pointer_t<hipStream_t>, CudaStreamDeleter>;
...
static CudaTensor createEmptyCudaData(size_t size) {
  uint8_t* ptr;
  TP_CUDA_CHECK(hipMalloc(&ptr, size));
...
static CudaTensor createFullCudaData(size_t size) {
  ...
  TP_CUDA_CHECK(hipMemcpy(ptr, data.get(), size, hipMemcpyHostToDevice));
  return CudaTensor(ptr);
}

static CudaStream createCudaStream() {
  hipStream_t stream;
  TP_CUDA_CHECK(hipStreamCreateWithFlags(&stream, hipStreamNonBlocking));
  ...
  for (int iterIdx = 0; iterIdx < numWarmUps + numRoundTrips; iterIdx++) {
    ...
    if (iterIdx >= numWarmUps) {
      ...
      if ((iterIdx - numWarmUps + 1) % data.cudaSyncPeriod == 0) {
        TP_CUDA_CHECK(hipStreamSynchronize(data.cudaStream.get()));
