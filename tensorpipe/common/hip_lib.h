...
#define TP_CUDA_DRIVER_CHECK(cuda_lib, a)                                 \
  do {                                                                    \
    ...
    if (error != hipSuccess) {                                          \
      ...
      TP_THROW_ASSERT_IF(res != hipSuccess);                            \
...
#define TP_FORALL_CUDA_SYMBOLS(_)                               \
  _(ctxGetCurrent, hipCtxGetCurrent, (hipCtx_t*))               \
  _(ctxSetCurrent, hipCtxSetCurrent, (hipCtx_t))                \
  _(deviceGet, hipGetDevice, (hipDevice_t*, int))                   \
  _(deviceGetCount, hipGetDeviceCount, (int*))                   \
  _(deviceGetUuid, cuDeviceGetUuid, (CUuuid*, hipDevice_t))        \
  _(getErrorName, hipGetErrorName___, (hipError_t, const char**))     \
  _(getErrorString, hipGetErrorString___, (hipError_t, const char**)) \
  _(init, hipInit, (unsigned int))                               \
  _(memGetAddressRange_v2,                                      \
    cuMemGetAddressRange_v2,                                    \
    (hipDeviceptr_t*, size_t*, hipDeviceptr_t))                       \
  _(pointerGetAttribute,                                        \
    hipPointerGetAttribute,                                      \
    (void*, CUpointer_attribute, hipDeviceptr_t))
...
#define TP_DECLARE_FIELD(method_name, function_name, args_types) \
  hipError_t(*function_name##_ptr_) args_types = nullptr;
...
#undef TP_LOAD_SYMBOL
  ...
  hipError_t memGetAddressRange(
      hipDeviceptr_t* pbase,
      size_t* psize,
      hipDeviceptr_t dptr) const {
    // NOTE: We are forwarding to cuMemGetAddressRange_v2() directly, because
    // the name hipMemGetAddressRange is #defined to its _v2 variant in cuda.h.
    // Calling the actual hipMemGetAddressRange() function here would lead to a
