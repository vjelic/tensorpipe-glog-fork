...
#include <tensorpipe/common/hip_lib.h>
...
#define TP_CUDA_CHECK(a)                                                \
  do {                                                                  \
    hipError_t error = (a);                                            \
    TP_THROW_ASSERT_IF(hipSuccess != error)                            \
        << __TP_EXPAND_OPD(a) << " " << hipGetErrorName(error) << " (" \
...
class CudaError final : public BaseError {
 public:
  explicit CudaError(hipError_t error) : error_(error) {}

  std::string what() const override {
    return std::string(hipGetErrorString(error_));
  }
...
class CudaDeviceGuard {
 public:
  ...
  explicit CudaDeviceGuard(int device) {
    TP_CUDA_CHECK(hipGetDevice(&device_));
    TP_CUDA_CHECK(hipSetDevice(device));
  }

  ~CudaDeviceGuard() {
    TP_CUDA_CHECK(hipSetDevice(device_));
  }
...
class CudaEvent {
 public:
  ...
  explicit CudaEvent(int device, bool interprocess = false)
    ...
    TP_CUDA_CHECK(hipEventCreateWithFlags(&ev_, flags));
  }

  explicit CudaEvent(int device, hipIpcEventHandle_t handle)
      : deviceIdx_(device) {
    // It could crash if we don't set device when creating events from handles
    CudaDeviceGuard guard(deviceIdx_);
    TP_CUDA_CHECK(hipIpcOpenEventHandle(&ev_, handle));
  }

  void record(hipStream_t stream) {
    CudaDeviceGuard guard(deviceIdx_);
    TP_CUDA_CHECK(hipEventRecord(ev_, stream));
  }

  void wait(hipStream_t stream, int device) {
    CudaDeviceGuard guard(device);
    TP_CUDA_CHECK(hipStreamWaitEvent(stream, ev_, 0));
  }

  bool query() const {
    CudaDeviceGuard guard(deviceIdx_);
    hipError_t res = hipEventQuery(ev_);
  ...
  hipIpcEventHandle_t getIpcHandle() const {
    CudaDeviceGuard guard(deviceIdx_);
    hipIpcEventHandle_t handle;
    TP_CUDA_CHECK(hipIpcGetEventHandle(&handle, ev_));
    return handle;
  }

  std::string serializedHandle() {
    hipIpcEventHandle_t handle = getIpcHandle();
    return std::string(reinterpret_cast<const char*>(&handle), sizeof(handle));
  }

  ~CudaEvent() {
    CudaDeviceGuard guard(deviceIdx_);
    TP_CUDA_CHECK(hipEventDestroy(ev_));
  }
...
inline int cudaDeviceForPointer(const CudaLib& cudaLib, const void* ptr) {
  // When calling hipSetDevice(0) when device 0 hasn't been initialized yet
  // the CUDA runtime sets the current context of the CUDA driver to what's
  // apparently an invalid non-null value. This causes hipPointerGetAttributes
  ...
  // the reason we call hipSetDevice(0) at all, because at destruction they
...
inline CudaPinnedBuffer makeCudaPinnedBuffer(size_t length, int deviceIdx) {
  ...
  uint8_t* ptr;
  TP_CUDA_CHECK(hipHostMalloc(&ptr, length));
...
class CudaDeviceBuffer {
 public:
  ...
  CudaDeviceBuffer(size_t length, int deviceIdx) {
    CudaDeviceGuard guard(deviceIdx);
    uint8_t* ptr;
    TP_CUDA_CHECK(hipMalloc(&ptr, length));
  ...
  hipIpcMemHandle_t getIpcHandle() const {
    CudaDeviceGuard guard(deviceIdx());
    hipIpcMemHandle_t handle;
    TP_CUDA_CHECK(hipIpcGetMemHandle(&handle, ptr_.get()));
    return handle;
  }
...
class CudaIpcBuffer {
 public:
  ...
  CudaIpcBuffer(int deviceIdx, const hipIpcMemHandle_t& handle) {
    CudaDeviceGuard guard(deviceIdx);
    void* ptr;
    TP_CUDA_CHECK(
 ...
 private:
  struct Deleter {
    ...
    void operator()(uint8_t* ptr) {
      CudaDeviceGuard guard(deviceIdx);
      TP_CUDA_CHECK(hipIpcCloseMemHandle(ptr));
    }
  };
