...
#include <tensorpipe/channel/hip_ipc/constants.h>
...
std::unique_ptr<optional<CudaEvent>[]> createIpcEventArray(
  ...
  // cause). The deadlock tends to manifest as a hipStreamSynchronize call
  ...
  // clearly, let's add extra syncs here. (The bug is fixed in v460).
  {
    ...
    TP_CUDA_CHECK(hipDeviceSynchronize());
  }
  for (size_t idx = 0; idx < numEvents; idx++) {
    ...
    // hipEvent_t, just like PyTorch does. However here we explicitly want to
...
std::vector<hipIpcEventHandle_t> getIpcHandlesForEventArray(
    optional<CudaEvent> events[],
    size_t numEvents) {
  std::vector<hipIpcEventHandle_t> eventHandles(numEvents);
  for (size_t idx = 0; idx < numEvents; idx++) {
...
std::shared_ptr<ContextImpl> ContextImpl::create() {
  ...
  for (const auto& device : getCudaDevices(cudaLib)) {
    ...
    TP_CUDA_CHECK(hipGetDeviceProperties(&props, device.index));
    ...
    if (props.computeMode != hipComputeModeDefault) {
      TP_VLOG(4) << "CUDA IPC channel is not viable because CUDA device "
  ...
  if (didntExist) {
    ...
    for (size_t slotIdx = 0; slotIdx < kNumSlots; slotIdx++) {
      outbox.events[slotIdx].emplace(
          localDeviceIdx,
          *reinterpret_cast<const hipIpcEventHandle_t*>(
