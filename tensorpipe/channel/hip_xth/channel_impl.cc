...
void RecvOperation::process() {
  {
    ...
    TP_CUDA_CHECK(hipStreamWaitEvent(stream, startEvent, 0));
    TP_CUDA_CHECK(
        hipMemcpyAsync(ptr, srcPtr, length, hipMemcpyDeviceToDevice, stream));
  }
...
void ChannelImpl::writeDescriptor(SendOpIter opIter) {
  ...
  static_assert(std::is_pointer<hipEvent_t>::value, "");
  static_assert(std::is_pointer<hipStream_t>::value, "");
