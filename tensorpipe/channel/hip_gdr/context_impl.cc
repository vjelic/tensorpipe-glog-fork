...
#include <tensorpipe/channel/hip_gdr/context_impl.h>
...
#include <tensorpipe/channel/hip_gdr/channel_impl.h>
#include <tensorpipe/channel/hip_gdr/error.h>
...
std::string getPciPathForGpu(int gpuIdx) {
  ...
  TP_CUDA_CHECK(
      hipDeviceGetPCIBusId(pciDeviceId.data(), pciDeviceId.size(), gpuIdx));
...
std::vector<std::string> matchGpusToIbvNics(
  ...
  int numGpus;
  TP_CUDA_CHECK(hipGetDeviceCount(&numGpus));
...
bool allGpusHaveEnoughBar1Size() {
  ...
  for (int gpuIdx = 0; gpuIdx < numGpus; gpuIdx++) {
    ...
    TP_CUDA_CHECK(hipGetDeviceProperties(&gpuProps, gpuIdx));
...
IbvMemoryRegion& IbvNic::registerMemory(CudaBuffer buffer) {
  ...
  TP_CUDA_DRIVER_CHECK(
      ...
      cudaLib_.memGetAddressRange(
          &basePtr, &allocSize, reinterpret_cast<hipDeviceptr_t>(buffer.ptr)));

  unsigned long long bufferId;
  TP_CUDA_DRIVER_CHECK(
      cudaLib_,
      cudaLib_.pointerGetAttribute(
          &bufferId, hipPointerAttributeBufferId, basePtr));
